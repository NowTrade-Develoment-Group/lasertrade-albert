...
if (process.env.NODE_ENV !== 'production') {
      url='http://srv552650.hstgr.cloud:80/admin'
}

else {
      url= 'http://backend.lasertrader.co/admin'  
}
...
const WatchList = () => {
  ...
  const getRealtimeData = async () => {
    try {
      const response = await axios.post('http://srv552650.hstgr.cloud:80/api/realtimedata', forexData);
